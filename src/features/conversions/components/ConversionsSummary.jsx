...
const renderCustomizedLabel = (props) => {
    ...
    return (
        <g>
            ...
            <text x={tx} y={ty - 8} textAnchor={textAnchor} fill="currentColor" fontSize={12} dominantBaseline="middle">
                {payload.level}
            </text>
...
const ConversionsSummary = ({
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Summary Error */}
            {summaryError && (
                <ErrorMessage message={summaryError.message || "Failed to load summary data"} onRetry={onRetry} />
            )}
                                ...
                                {summaryData.totalConversions > 0 && (
                                    <p className="text-xs text-muted-foreground mt-1">
                                        {summaryData.adInfluencedCount > 0
                                            ? `${Math.round(
                                                  (summaryData.adInfluencedCount / summaryData.totalConversions) * 100,
                                              )}% of total`
                                            : "0% of total"}
                                    </p>
                                )}
                                ...
                                {summaryData.totalRevenue > 0 && (
                                    <p className="text-xs text-muted-foreground mt-1">
                                        {summaryData.adsRevenue > 0
                                            ? `${Math.round(
                                                  (summaryData.adsRevenue / summaryData.totalRevenue) * 100,
                                              )}% of revenue`
                                            : "0% of revenue"}
                                    </p>
                                )}
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="grid grid-cols-1 gap-6">
                            {/* Pie Chart */}
                            <div
                                className={`${isMobile ? "h-64" : "h-96"} flex items-center justify-center`}
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    ...
                                    >
                                        ...
                                        <Legend
                                            ...
                                            iconType="circle"
                                            formatter={(value) => (
                                                <span className={`text-foreground ${isMobile ? "text-xs" : "text-sm"}`}>
                                                    {value}
                                                </span>
                                            )}
                                            wrapperStyle={{
                                    ...
                                    <table className="w-full min-w-[600px] text-sm">
                                        <thead>
                                            <tr className="border-b">
                                                ...
                                                <th className="text-right pb-2 px-4 whitespace-nowrap">Count</th>
                                                <th className="text-right pb-2 px-4 whitespace-nowrap">Percentage</th>
                                                <th className="text-right pb-2 px-4 whitespace-nowrap">
                                                    Avg Influence
                                                </th>
                                                <th className="text-right pb-2 pl-4 whitespace-nowrap">Revenue</th>
                                            </tr>
                                        </thead>
                                        <tbody>
