...
const ComparisonModal = ({ isOpen, onClose, currentPeriod, sourceFilter }) => {
    ...
    useEffect(() => {
        ...
        customStartDate,
        customEndDate,
        customCurrentStartDate,
        customCurrentEndDate,
        memoizedCurrentPeriod,
        sourceFilter,
    ]);
                                ...
                                {customCurrentStartDate &&
                                    ...
                                    (() => {
                                        try {
                                            ...
                                            const hasOverlap =
                                                currentStart.isSameOrBefore(compEnd, "day") &&
                                                currentEnd.isSameOrAfter(compStart, "day");

                                            return hasOverlap ? (
                                                <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md p-3">
                                                    <p className="text-sm text-yellow-800 dark:text-yellow-200">
                                                        ⚠️ Warning: The selected periods overlap. This may affect
                                                        comparison accuracy.
                                                    </p>
                                                </div>
                                            ) : null;
                    ...
                    <div className="mb-6 p-4 bg-muted/50 rounded-lg space-y-2">
                        <p className="text-sm text-muted-foreground">
                            <span className="font-medium">
                                {comparisonRange === COMPARISON_RANGES.LAST_WEEK
                                    ? "Last week:"
                                    : comparisonRange === COMPARISON_RANGES.LAST_MONTH
                                      ? "Current month:"
                                      : comparisonRange === COMPARISON_RANGES.LAST_7_DAYS
                                        ? "Last 7 days:"
                                        : comparisonRange === COMPARISON_RANGES.LAST_14_DAYS
                                          ? "Last 14 days:"
                                          : "Current period:"}
                            </span>{" "}
                            {dayjs(getAdjustedCurrentPeriod().startDate).format("MMM D, YYYY")} -{" "}
                            {dayjs(getAdjustedCurrentPeriod().endDate).format("MMM D, YYYY")}
                        </p>
                        {(comparisonData ||
                            (comparisonRange === COMPARISON_RANGES.CUSTOM && customStartDate && customEndDate)) && (
                            <p className="text-sm text-muted-foreground">
                                <span className="font-medium">
                                    {comparisonRange === COMPARISON_RANGES.LAST_WEEK
                                        ? "Previous week:"
                                ...
                                {dayjs(getComparisonPeriod().startDate).format("MMM D, YYYY")} -{" "}
                                {dayjs(getComparisonPeriod().endDate).format("MMM D, YYYY")}
                            </p>
                        )}
                    </div>
