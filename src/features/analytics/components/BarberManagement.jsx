...
const BarberManagement = () => {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            showToast(`Failed to delete team member: ${err.response?.data?.error || err.message}`, "error");
        } finally {
            setIsDeleting(false);
        }
    };
    ...
    const handleAutoSort = async () => {
        ...
        try {
            ...
            if (err.response && err.response.status === 401) {
                errorMessage =
                    "Auto-sort failed: Please refresh the page and log in again. Your session may have expired.";
            } else if (err.message && err.message.includes("fetch analytics")) {
                errorMessage =
                    "Auto-sort failed: Unable to load analytics data. Please ensure you're logged in and try again.";
            } else {
    ...
    const getEmploymentBadge = (employmentType) => {
        ...
        return <Badge variant={variants[employmentType]}>{labels[employmentType]}</Badge>;
    };
            ...
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                        Drag and drop to reorder barbers • Auto-sort by employment & conversions (with date range) •
                                                                ...
                                                                <>
                                                                    <tr
                                                                        ...
                                                                        className={`border-b transition-colors ${
                                                                            snapshot.isDragging
                                                                                ? "shadow-lg bg-background relative z-10"
                                                                                : "hover:bg-muted/50"
                                                                        }`}
                                                                    >
                                                                                ...
                                                                                <td className="p-4 align-middle text-center">
                                                                                    <div className="flex gap-1 justify-center flex-wrap">
                                                                                        ...
                                                                                        <Button
                                                                                            ...
                                                                                            variant="outline"
                                                                                            onClick={() =>
                                                                                                handleOpenServiceAssignmentInfo(
                                                                                                    member,
                                                                                                )
                                                                                            }
                                                                                            className="h-8 px-3"
